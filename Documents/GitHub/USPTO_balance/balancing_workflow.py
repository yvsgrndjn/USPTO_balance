...
def convert_and_save_subset(subset, subset_mol, retro_reac, foldername: str = ''):
    if subset:
        name = f'GDB13S_sub_{retro_reac}'
        folder_path = f'./GDB_subsets_{foldername}'
        ...
        subset_mol = [subset_mol[i] for i in range(len(subset)) if subset[i]]

        folder_path_mol = f'./GDB_subsets_mol_{foldername}'
...
def extract_match_smiles_from_dataset(dataset:list, dataset_mol:list, template:str):
    """
    This function extracts the elements from a smiles dataset that match a certain template
    """
    template_mol  = Chem.MolFromSmarts(template)
    match_ind     = [mol.HasSubstructMatch(template_mol) for mol in dataset_mol]
    return [dataset[i] for i in range(len(dataset)) if match_ind[i] == True], match_ind


#(probably obsolete)
#def apply_rxn_template_on_smiles_list(dataset:list, rxn_template:str):
#    '''
#    Applies a reaction template on a list of smiles (dataset) and returns a list of lists of reactants
#
#    dataset: list of molecules smiles 
...
#    return [rxn.RunReactants((Chem.MolFromSmiles(dataset[i]),)) for i in range(len(dataset))]
